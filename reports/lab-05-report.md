...
- `strace` showed:
C 코드에서는 open()을 썼는데 strace에서는 openat()으로 보임
-> 이유: 현대 Linux에서는 라이브러리 층에서 open()이 내부적으로 openat()계열로 이어지는 경우가 흔함
- Error case I tested: 
1. 존재하지 않는 원본 파일의 경우: ./copy_syscall nofile.txt out.txt
2. 디렉터리를 원본처럼 넣은 경우: ./copy_syscall . out.txt
